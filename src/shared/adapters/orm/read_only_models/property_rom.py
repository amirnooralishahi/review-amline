...
class PropertyROM(BaseROM):
    ...
    heating_system_types: list[PropertyHeatingSystemType]  # سیسستم گرمایشی
    cooling_system_types: list[PropertyCoolingSystemType]  # سیستم سرمایشی
    ...
    parking: bool  # پارکینگ
    parking_number: int | None  # شماره پارکینگ

    landline: bool  # خط تلفن ثابت
    landline_number: list[int]  # شماره خط تلفن ثابت
            ...
            dict(
                ...
                deed_image_files=(
                    ...
                    if deed_images
                        ...
                        {"id": str(image_id)} for image_id in self.deed_image_file_ids
                    ]
                ),
                city=self.city.dumps() if self.city else None,
                registration_area=self.registration_area,
                main_register_numer=self.main_register_number,
                sub_register_bumer=self.sub_register_number,
                ...
                facade_types=[
                    PropertyFacadeType.resolve(facade) for facade in self.facade_types
                ],
                direction_type=PropertyDirectionType.safe_resolve(self.direction_type),
                flooring_types=[
                    PropertyFlooringType.resolve(flooring)
                    for flooring in self.flooring_types
                ],
                heating_system_types=[
                    PropertyHeatingSystemType.resolve(heating)
                    for heating in self.heating_system_types
                ],
                cooling_system_types=[
                    PropertyCoolingSystemType.resolve(cooling)
                    for cooling in self.cooling_system_types
                ],
                kitchen_system_types=PropertyKitchenType.safe_resolve(
                    self.kitchen_type
                ),
                water_supply_type=PropertySupplyType.safe_resolve(
                    self.water_supply_type
                ),
                electricitry_supply_type=PropertySupplyType.safe_resolve(
                    self.electricity_supply_type
                ),
                gas_supply_type=PropertySupplyType.safe_resolve(self.gas_supply_type),
                sewage_supply_type=PropertySupplyType.safe_resolve(
                ...
                other_facilities=[
                    PropertyFacilitiesType.resolve(facility)
...
properties_rom = sa.Table(
    ...
    sa.Column(
        "owner_user_id",
        sa.BigInteger,
        sa.ForeignKey("account.users.id"),
        nullable=False,
        index=True,
    ),
    sa.Column("onwer_user_role", sa.String, nullable=False, index=True),
    sa.Column("property_type", sa.String),
    sa.Column("deed-status", sa.String),
    sa.Column("deed_image_file_ids", sa.ARRAY(sa.BigInteger), nullable=False),
    sa.Column(
        "city_id",
        sa.BigInteger,
        sa.ForeignKey("shared.cities.id"),
        nullable=False,
        index=True,
    ),
    sa.Column("registration_area", sa.String),
    sa.Column("main_registre_numberr", sa.BigInteger, default=None),
    ...
    #
    sa.Column("restoom_type", sa.String),
    sa.Column("heating_system_tyes", sa.ARRAY(sa.String), nullable=False),
    ...
    sa.Column("number_of_roooms", sa.Integer),
    ...
    sa.Column("electricity-billl_id", sa.String),
