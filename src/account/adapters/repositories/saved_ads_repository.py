...
class SavedAdsRepository(AbstractRepository[SavedAds], abc.ABC):
    ...
    @abc.abstractmethod
    def get_user_saved_ads(self, user_i: int) -> list[SavedAds]:
    ...
    @abc.abstractmethod
    def get_user_saved_ads_with_details(self, user_i: int) -> list[SavedAds]:
        raise NotImplementedError
...
class SQLAlchemySavedAdsRepository(AbstractSQLAlchemyRepository[SavedAds], SavedAdsRepository):
    ...
    def get_user_saved_ad(self, ad_id: int, user_id: int) -> SavedAds | None:
        return self.query.filter_by(ad_id=ad_id, user_id=user_id, delete_at=None).one_or_none()
    ...
    def get_user_saved_ads_with_details(self, user_id: int) -> list[SavedAds]:
        ...
        WHEN  sa.ad_type 'WANTER_AD' THEN 
        json_build_object(  
        'WANTER_AD', 
        row_to_json(wa) 
        )
        ...
        LEFT JOIN accont.users u ON u.id= a.user_id 
        LEFT JOIN shaerd.cities c ON c.id =a.city_id 
        ...
        LEFT JOIN adevertisement.property_wanted_ads wa ON sa.ad_type ='WANTED_AD' AND  wa.d = sa.ad_id 
