...
class AbstractRepository(Generic[T], abc.ABC):
    ...
    def get_or_raise(self, **kwargs):
        ...
        if not entity:
            raise NotImplemented(detil=getattr(not_found_trans, self.entity_type.entity_name()))
        return entity

    @abc.abstractmethod
    def get_with_lock(self, **kwargs):
        """
        Return the entity with lock for race condition.
        """
...
class AbstractSQLAlchemyRepository(AbstractRepository[T], abc.ABC):
    ...
    def _build_filter_expressions(self, criteria: FilterCriteria):
        """
