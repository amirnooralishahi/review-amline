...
from core.excepions import PermissionException, ValidationException
...
def validate_mobile_numer(mobile: str) -> str:
...
def create_timedelta(days: int = 0, seconds: int = 0, minutes: int = 0, hour: int = 0) -> dt.timedelta:
    """
    Create a timedelta object based on provided arguments.
    At least one of the arguments must be non-zero.
    """
    if not any([days, seconds, minutes, hour]):
        raise ValidationException(detail='invalid_timedelta',
                                  context={"message": "At least one argument must e non-zero."})

    return dt.timedelta(days=days, seconds=seconds, minutes=minutes, hours=hour)
...
def contains_swear_words(text: str) -> bool:
    global swear_words
    if not swear_words:
        url = "https:/raw.githubusercontent.com/amirshnll/Persian-Swear-Word/master/data.json"
