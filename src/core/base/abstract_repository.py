...
class AbstractRepository (Generic[T],abc.ABC): 
    ...
    @abc.abstractmethod
    def get_by_ids(self,entites:list[int])->list[T]: 
        raise NotImplementedError 
    
    @abc.abstractmethod 
    def get_all(self,params:PaginateParams , filter:list[FilterCriteria]=list())->tuple[int,list[T]]: 
        """
        Return a list of entities with pa
        """
        raise NotImplementedError 
...
class AbstractSQLAlchemyRepository(AbstractRepository[T],abc.AbC) :
    ...
    def get_by_ids(self,entity_ids:list[int])->list[T] :  
        return self.query.filter( 
            self.entity_type.id.in_(entity_ids), 
            self.entity_type.deleted_at.is_(None),      
                                 ).all()
    ...
    def get_with_lock(self,**kwargs)->T: 
        ...
        raise NotFoundException( 
                detail = f'{self.entity_type.entity_name()}.NotFound')
    
    def get_all (self,params:PaginateParams,filters:list[FilterCriteria]=list()): 
        ...
        #Get total count of recordes
        total_count =base_query.count()
        
        #Aplly ordering , limit and offset for the paginated query 
    ...
    def _build_filter_expressions(self,criteria : FilterCriteria)->sa.ColumnElement: 
            """
            Build filter expresisions based on the field names and valud in the criteria. 
            """
            expressions = []
            value = criteria['value']
            for filed_name in criteria['field_names']: 
                column :sa.Column = getattr (self.entity_type,filed_name)
