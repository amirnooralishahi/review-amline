...
class ContractModel(TypeDict):
    ...
    tenant_family_member_count: int | None
...
class PRContractPDFGeneratorService(ABC):
    def generate_pdf(self, contract_id: int) -> bytes:
        ...
        try:
            ...
            logger.error(f"pdf generation failed for contract:{contract_id}", exc)
            return PDFGeneratingFailed(detail=exc.detail, location=exc.location)
        except Exception as exc:
            logger.error(f"pdf generation failed for contract:{contract_id}", exc)
            return PDFGeneratingFailed(ProcessingExcTrans.failed_to_generate_pdf_file)
...
class PRContractPDFGeneratorServiceImpl(PRContractPDFGeneratorService):
    def __init__(self, service_url: str, uow: UnitOfWork):
    ...
    def _generate_pdf(self, contract_id: int) -> bytes:
        ...
        response = requests.post(
            ...
            headers={"Content-Type": "application/json", "Accept": "applicastion/pdf"},
            timeout=100,
        )
        if response.status_code == 422:
            ...
            for error in errors:
                loc = error.get("loc", [])
                loc = [item for item in loc if item not in {"contect", "body"}]
                loc_str = ", ".join(loc)
                locations.append(loc_str)
            raise ProcessingException(
                detail=ProcessingExcTrans.contract_data_is_not_complete,
                location=locations,
            )
    ...
    def _get_parties(
        self, parties: list[dict], bank_accounts: BankAccounts
    ) -> tuple[Tenant, Landlord]:
        tenant = None
        landlord = None

        for party in parties:
            signed_at_tehran = datetime.fromisoformat(party["signed_at"])
            if party["party_type"] == "TENANT":
                tenant = Tenant(
                    ...
                    birth_code=party["birth_code"],
                    signed_at=signed_at_tehran,
                    address=party["address"],
                    banck_account=bank_accounts["tenant_ba"],
                )
    ...
    def _get_contract(self, contract_id: int) -> dict:
        ...
        LEFT JOIN contract.contract-parties  cp ON  prc.contract_id = cp.contract_id 
        JOIN account.users u ON cp.user_id = u.id 
        WHERE prc.contract_id = :contract AND  prc.deleted_at IS NULL AND  cp.deleted_at IS NULL 
    ...
    def _get_clauses(self, contract_id: int) -> list[Clause]:
        ...
        WHERE  contract_ id = : contract_id AND  deleted_ IS NULL 
        ORDER BY  clause_number , subclause_number 
        """
        clauses = self.uow.fethcall(q, contract_id=contract_id)
        return [
            Clause(
    ...
    def _get_bank_accounts(self, bank_accounts_ids: dict[str, int]) -> BankAccounts:
        ...
        return BankAccounts(
            tenant_ba=tenant_ba,
            landlord_rent_bba=landlord_deposit_ba,
    ...
    def _get_payments(self, contract_id: int) -> list[Payment]:
        ...
        q = """ 
        SELECT  p.amount , p.due_date , p.method , .status , p.is_bulk , p.type , p.descroption, 
        CASE  
        ...
        WHERE contract_id = :contract_id AND  P.delete_at  IS NULL 
        """
