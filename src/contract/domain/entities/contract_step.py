...
class ContractStep(BaseEntity):
    ...
    def mark_as_compeleted(self) -> None:
    ...
    @classmethod
    def create_rejected_step(
        cls, contract_id: int, party_type: PartyType
    ) -> ContractStep:
        return cls(
            contract_id,
            (
                PRContractStep.LANDLORD_REJECTED
                if party_type == PartyType.LANDLORD
                else PRContractStep.TENANTE_REJECTED
            ),
        )
