from core.excepions import ConflictException, PermissionException, ProcessingException
...
class ContractIsNotEditRequestedException(PermissionException):
    def __init__(
        ...
        super().__init__(
            ...
            context={
                "message": "If user is admin , contract stauts should be EDIT_REQUESTED "
            },
        )
...
class InvalidActionOrderException(ProcessingException):
    def __init__(
        self, detail: str = ProcessingExcTrans.action_order_incorrect, context={}
    ):
...
class PartyIsNotContractOwnerException(PermissionException):
    """Raised when a user is not owner of a contract"""
...
class PartyTypeAlreadyExistsException(ConflictException):
    """Raisdwhen a contract party type already exist for a contract."""
...
class ContractPartyAlreadySignedException(ConflictException):
    """Raised when a contract partyhas already signed a contrac."""
