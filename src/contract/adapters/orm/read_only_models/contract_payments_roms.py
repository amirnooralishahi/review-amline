...
class ContractPaymentROM(BaseROM):
    ...
    def dumps(
        self, bank_account: dict[str, BankAccontROM | None] = dict(), **kwargs
    ) -> dict:
        payer_bank_account, payee_bank_account = self._handle_bank_account(bank_account)
        ...
        return {
            ...
            "invoice": kwargs.get(
                "invoice", {"id": str(self.invoice_id)} if self.invoice_id else None
            ),
            "amount": self.amount,
            "method": self.method,
            "type": self.type,
    ...
    def _handle_bank_accounts(
        self, bank_accounts: dict[str, BankAccontROM | None] = dict()
    ) -> tuple:
        tenant_bank_account = bank_accounts.get("tenant_bank_account")
        landlord_rent_bank_accont = bank_accounts.get("landlord_rent_bank_account")
        landlord_deposit_bank_account = bank_accounts.get(
        ...
        if self.type == enums.PaymentType.RENT:
            payer_bank_account = (
                tenant_bank_account.dumps()
                if tenant_bank_account
                else {"user_id": str(self.payer_id)}
            )
            payee_bank_account = (
                landlord_rent_bank_accont.dumps()
                if landlord_rent_bank_accont
            ...
            if tenant_bank_account and self.payer_id == tenant_bank_account.user_id:
                ...
                landlord_rent_bank_accont
                and self.payer_id == landlord_rent_bank_accont.user_id
            ):
                payer_bank_account = landlord_rent_bank_accont.dumps()
            elif (
                landlord_deposit_bank_account
                and self.payer_id == landlord_deposit_bank_account
            ):
...
contract_payments_rom = sa.Table(
    ...
    sa.Column(
        "owner_id", sa.BigInteger, sa.ForeignKey("account.users.id"), nullable=False
    ),
    sa.Column(
        "contract_id",
        sa.BigInteger,
    ...
    sa.Column(
        "invoice_id",
        sa.BigInteger,
        sa.ForeignKey("financial.invoices.id"),
        nullable=False,
    ),
    sa.Column(
        "payer_id",
    ...
    sa.Column("amoun", sa.BigInteger, nullable=False),
