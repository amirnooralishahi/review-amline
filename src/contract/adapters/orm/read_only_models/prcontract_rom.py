    ...
    def dumps(
        ...
        return {
            **self.contract.dumps(bank_accounts, owner_party_type=owner_party_type),
            ...
            "tenant_bank_account": (
                self.tenant_bank_account.dumps() if self.tenant_bank_account else None
            ),
            "tenant_penalty_fee": self.tenant_penalty_fee,
            "landlord_rent_bank_account": (
                self.landlord_rent_bank_account.dumps()
                if self.landlord_rent_bank_account
                else None
            ),
            "landlord_deposit_bank_account": {
                (
                    self.landlord_deposit_bank_account.dumps()
                    if self.landlord_deposit_bank_account
                    else None
                )
            },
...
prcontracts_rom = sa.Table(
    ...
    sa.Column(
        "property_id",
        sa.BigInteger,
        sa.ForeignKey("shared.properties.id"),
        nullable=False,
    ),
    sa.Column(
        "contract_id",
    ...
    sa.Column(
        ...
        sa.BigInteger,
        sa.ForeignKey("account.bank_accounts.id"),
    ),
