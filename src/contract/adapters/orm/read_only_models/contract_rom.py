...
class ContractROM(BaseROM):
    ...
    clauses: list[ConractClausesROM]
    paymetns: list[ContractPaymentROM]
    deleted_at: datetime | None = None

    def dumps(
        self,
        bank_accounts: dict[str, BankAccountROM | None] = dict(),
        owner_party_type: enums.PartyType | None = None,
    ) -> dict:
        ...
        return dict(
            id=str(
                self.id0,
                owner={
                    "id": str(self.owner_party_type),
                    "party_type": self.owner_party_type or owner_party_type,
                },
                type=self.type,
                status=self.status,
                date=self.date,
                ...
                payments=[payment.dumps(bank_accounts) for payment in self.paymetns],
            )
        )
    ...
    def list_dumps(self) -> dict:
        return dict(
            ...
            status=self.status,
            date=self.date,
            parties=[party.dump() for party in self.parties],
        )
...
contracts_rom = sa.Table(
    ...
    sa.Column(
        "owner_user_id", sa.Integer, sa.ForeignKey("account.users.id"), nullable=False
    ),
