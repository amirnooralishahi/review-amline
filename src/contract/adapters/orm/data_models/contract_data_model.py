...
contracts = sa.Table(
    ...
    sa.Column("update_at", sa.DateTime(timezone=True), default=sa.func.now()),
    sa.Column("deleted_at", sa.DateTime(timezone=True)),
    sa.Column(
        "created_by",
        sa.BigInteger,
        sa.ForeignKey("accounts.users.id"),
        nullable=False,
        index=True,
    ),
    sa.Column("password", sa.String(8)),
    sa.Column(
        "pdf_file_id", sa.BigInteger, sa.ForeignKey("sahred.files_.id"), index=True
    ),
    schema="contract",
)
